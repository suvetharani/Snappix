...
export default function Messages() {
  const [showSidebar, setShowSidebar] = useState(() => window.innerWidth <= 350);
  const [isTinyScreen, setIsTinyScreen] = useState(() => window.innerWidth <= 350);

  useEffect(() => {
    const handleResize = () => {
      setIsTinyScreen(window.innerWidth <= 350);
      if (window.innerWidth > 350) setShowSidebar(true);
    };
  ...
  const selectChat = async (user) => {
    ...
    if (window.innerWidth <= 350) setShowSidebar(false);
  };
